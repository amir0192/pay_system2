...
async def login_user(phone_number: int = Body, password: str = Body):
    ...
    # если данные верны отправляем в юзер
    return {'status': 1, 'message': 'Login_on'}

# Добавление карты в базу
@app.post('/api/add-card')
async def add_user_card(card_data: CardDent):
    # Вызов функции из бд для добавление карты в базу
    result = card_data
    print(card_data)

    # Если успешно добавление карты, то status 1
    return {'status': 1, 'message': result}

# выход данных о пользователя
